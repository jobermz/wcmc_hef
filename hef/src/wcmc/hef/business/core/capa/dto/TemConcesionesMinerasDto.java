package wcmc.hef.business.core.capa.dto;

import java.util.Date;
import java.util.List;
import wcmc.hef.general.util.CadenaUtil;

public class TemConcesionesMinerasDto implements BaseBeanVectorialDto {
    private String strHashConsulta;
	private Integer srlGid;
	private String strTheGeom;
	private Integer intObjectid1;
	private String strGmlId;
	private Integer intObjectid;
	private String strCodigou;
	private String strZona;
	private String strConcesion;
	private String strEstado;
	private String strTitConces;
	private String strNaturaleza;
	private String strFecDenu;
	private Double dblHectarea;
	private String strCarta;
	private String strNmDist;
	private String strNmProv;
	private String strNmDepa;
	private Double dblShapeStar;
	private Double dblShapeStle;

    public void setStrHashConsulta(String strHashConsulta) {
        this.strHashConsulta = strHashConsulta;
    }
	
    public String getStrHashConsulta() {
        return CadenaUtil.getStr(strHashConsulta);
    }
	public Integer getSrlGid() {
		return srlGid;
	}
	public void setSrlGid(Integer srlGid) {
		this.srlGid = srlGid;
	}
	public String getStrTheGeom() {
		return strTheGeom;
	}
	public void setStrTheGeom(String strTheGeom) {
		this.strTheGeom = strTheGeom;
	}
	public Integer getIntObjectid1() {
		return intObjectid1;
	}
	public void setIntObjectid1(Integer intObjectid1) {
		this.intObjectid1 = intObjectid1;
	}
	public String getStrGmlId() {
		return strGmlId;
	}
	public void setStrGmlId(String strGmlId) {
		this.strGmlId = strGmlId;
	}
	public Integer getIntObjectid() {
		return intObjectid;
	}
	public void setIntObjectid(Integer intObjectid) {
		this.intObjectid = intObjectid;
	}
	public String getStrCodigou() {
		return strCodigou;
	}
	public void setStrCodigou(String strCodigou) {
		this.strCodigou = strCodigou;
	}
	public String getStrZona() {
		return strZona;
	}
	public void setStrZona(String strZona) {
		this.strZona = strZona;
	}
	public String getStrConcesion() {
		return strConcesion;
	}
	public void setStrConcesion(String strConcesion) {
		this.strConcesion = strConcesion;
	}
	public String getStrEstado() {
		return strEstado;
	}
	public void setStrEstado(String strEstado) {
		this.strEstado = strEstado;
	}
	public String getStrTitConces() {
		return strTitConces;
	}
	public void setStrTitConces(String strTitConces) {
		this.strTitConces = strTitConces;
	}
	public String getStrNaturaleza() {
		return strNaturaleza;
	}
	public void setStrNaturaleza(String strNaturaleza) {
		this.strNaturaleza = strNaturaleza;
	}
	public String getStrFecDenu() {
		return strFecDenu;
	}
	public void setStrFecDenu(String strFecDenu) {
		this.strFecDenu = strFecDenu;
	}
	public Double getDblHectarea() {
		return dblHectarea;
	}
	public void setDblHectarea(Double dblHectarea) {
		this.dblHectarea = dblHectarea;
	}
	public String getStrCarta() {
		return strCarta;
	}
	public void setStrCarta(String strCarta) {
		this.strCarta = strCarta;
	}
	public String getStrNmDist() {
		return strNmDist;
	}
	public void setStrNmDist(String strNmDist) {
		this.strNmDist = strNmDist;
	}
	public String getStrNmProv() {
		return strNmProv;
	}
	public void setStrNmProv(String strNmProv) {
		this.strNmProv = strNmProv;
	}
	public String getStrNmDepa() {
		return strNmDepa;
	}
	public void setStrNmDepa(String strNmDepa) {
		this.strNmDepa = strNmDepa;
	}
	public Double getDblShapeStar() {
		return dblShapeStar;
	}
	public void setDblShapeStar(Double dblShapeStar) {
		this.dblShapeStar = dblShapeStar;
	}
	public Double getDblShapeStle() {
		return dblShapeStle;
	}
	public void setDblShapeStle(Double dblShapeStle) {
		this.dblShapeStle = dblShapeStle;
	}
	
}
